...
int g_fg_battery_id_volt = 0;
#ifdef MTK_GET_BATTERY_ID_BY_AUXADC
void fgauge_get_profile_id(void)
{
	int ret = 0;

	ret = IMM_GetOneChannelValue_Cali(BATTERY_ID_CHANNEL_NUM, &g_fg_battery_id_volt);
	if (ret != 0)
		bm_print(BM_LOG_CRTI, "[fgauge_get_profile_id]g_fg_battery_id_volt read fail\n");
	else
		bm_print(BM_LOG_CRTI, "[fgauge_get_profile_id]g_fg_battery_id_volt = %d\n", g_fg_battery_id_volt);
	...
	if(((g_battery_id_voltage[0] - 50000 ) < g_fg_battery_id_volt) && (g_fg_battery_id_volt < (g_battery_id_voltage[0] + 50000))){
		g_fg_battery_id = 1;
	}
	else
	{
	...
	for (id = 0; id < TOTAL_BATTERY_NUMBER; id++) {
		if (g_fg_battery_id_volt < g_battery_id_voltage[id]) {
			g_fg_battery_id = id;
			break;
...
//[BUGFIX]-Mod-BEGIN by SCDTABLET.zhengding.chen@tcl.com,01/12/2016,PR-1401256
//compatible SCUD&JN battery, SCUD id=1, JN id=0 
	...
	{
		__batt_meter_parse_node(np, "battery_profile_id1_t0_num", &num);

		__batt_meter_parse_table(np, "battery_profile_id1_t0",
				fgauge_get_profile(batt_meter_cust_data.temperature_t0));

		__batt_meter_parse_node(np, "battery_profile_id1_t1_num", &num);

		__batt_meter_parse_table(np, "battery_profile_id1_t1",
				fgauge_get_profile(batt_meter_cust_data.temperature_t1));

		__batt_meter_parse_node(np, "battery_profile_id1_t2_num", &num);

		__batt_meter_parse_table(np, "battery_profile_id1_t2",
				fgauge_get_profile(batt_meter_cust_data.temperature_t2));

		__batt_meter_parse_node(np, "battery_profile_id1_t3_num", &num);

		__batt_meter_parse_table(np, "battery_profile_id1_t3",
				fgauge_get_profile(batt_meter_cust_data.temperature_t3));

		__batt_meter_parse_node(np, "r_profile_id1_t0_num",	&num);

		__batt_meter_parse_table(np, "r_profile_id1_t0",
				(BATTERY_PROFILE_STRUCT *)fgauge_get_profile_r_table(batt_meter_cust_data.temperature_t0));

		__batt_meter_parse_node(np, "r_profile_id1_t1_num",	&num);

		__batt_meter_parse_table(np, "r_profile_id1_t1",
				(BATTERY_PROFILE_STRUCT *)fgauge_get_profile_r_table(batt_meter_cust_data.temperature_t1));

		__batt_meter_parse_node(np, "r_profile_id1_t2_num",	&num);

		__batt_meter_parse_table(np, "r_profile_id1_t2",
				(BATTERY_PROFILE_STRUCT *)fgauge_get_profile_r_table(batt_meter_cust_data.temperature_t2));

		__batt_meter_parse_node(np, "r_profile_id1_t3_num",	&num);

		__batt_meter_parse_table(np, "r_profile_id1_t3",
				(BATTERY_PROFILE_STRUCT *)fgauge_get_profile_r_table(batt_meter_cust_data.temperature_t3));
	}
	else
	{
		__batt_meter_parse_node(np, "battery_profile_id0_t0_num", &num);

		__batt_meter_parse_table(np, "battery_profile_id0_t0",
				fgauge_get_profile(batt_meter_cust_data.temperature_t0));

		__batt_meter_parse_node(np, "battery_profile_id0_t1_num", &num);

		__batt_meter_parse_table(np, "battery_profile_id0_t1",
				fgauge_get_profile(batt_meter_cust_data.temperature_t1));

		__batt_meter_parse_node(np, "battery_profile_id0_t2_num", &num);

		__batt_meter_parse_table(np, "battery_profile_id0_t2",
				fgauge_get_profile(batt_meter_cust_data.temperature_t2));

		__batt_meter_parse_node(np, "battery_profile_id0_t3_num", &num);

		__batt_meter_parse_table(np, "battery_profile_id0_t3",
				fgauge_get_profile(batt_meter_cust_data.temperature_t3));

		__batt_meter_parse_node(np, "r_profile_id0_t0_num",	&num);

		__batt_meter_parse_table(np, "r_profile_id0_t0",
				(BATTERY_PROFILE_STRUCT *)fgauge_get_profile_r_table(batt_meter_cust_data.temperature_t0));

		__batt_meter_parse_node(np, "r_profile_id0_t1_num",	&num);

		__batt_meter_parse_table(np, "r_profile_id0_t1",
				(BATTERY_PROFILE_STRUCT *)fgauge_get_profile_r_table(batt_meter_cust_data.temperature_t1));

		__batt_meter_parse_node(np, "r_profile_id0_t2_num",	&num);

		__batt_meter_parse_table(np, "r_profile_id0_t2",
				(BATTERY_PROFILE_STRUCT *)fgauge_get_profile_r_table(batt_meter_cust_data.temperature_t2));

		__batt_meter_parse_node(np, "r_profile_id0_t3_num",	&num);

		__batt_meter_parse_table(np, "r_profile_id0_t3",
				(BATTERY_PROFILE_STRUCT *)fgauge_get_profile_r_table(batt_meter_cust_data.temperature_t3));
	}
#endif
...
static ssize_t show_bat_id(struct device *dev, struct device_attribute *attr,
					 char *buf)
{
	bm_print(BM_LOG_CRTI, "[bat_id] bat_id=%d, bat_id_vol=%d\n", g_fg_battery_id, g_fg_battery_id_volt);
	return sprintf(buf, "bat_id=%d, bat_id_vol=%d\n", g_fg_battery_id, g_fg_battery_id_volt);
}
