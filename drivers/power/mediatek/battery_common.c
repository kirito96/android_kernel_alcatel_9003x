...
#endif
//[BUGFIX]-Mod-BEGIN by SCDTABLET.jinghuang@tcl.com,12/26/2015,1240366
//add UISOC algorithm for low temperature battery charging curve.
...
//[BUGFIX]-Mod-BEGIN by SCDTABLET.zhegnding.chen@tcl.com,01/29/2016,PR 1537389
//add charge enable node for enable or disable charging, sys/devices/platform/battery/chg_enable
int sys_chg_control = 1;
//[BUGFIX]-Mod-END by SCDTABLET.zhengding.chen@tcl.com
...
/*[BUGFIX]-Add-BEGIN by TCTSZ.zhengding.chen, 1/7/2016, pr-1368925, add battery manufacturer check */
#ifdef TCT_CHECK_BATTERY_TYPE
extern kal_uint32 g_fg_battery_id;
#endif
/*[BUGFIX]-Add-END by TCTSZ.zhengding.chen */
...
/* [PLATFORM]-Add-BEGIN by TCTSZ.pingao, 12/28/2015,  PR-1207804, change charge current */
#define CONFIG_TCT_LIMIT_USB_CURRENT
/* [PLATFORM]-Add-END by TCTSZ.pingao.yang, 12/28/2015 */
...
static kal_int32 get_sync_soc_time(kal_int32 soc)
{
	...
	printk("jinghuang:[get_sync_soc_time] soc=%d, sync_time=%d\n", soc, sync_time);
	return sync_time;
}
#endif
//[BUGFIX]-Mod-END by SCDTABLET.jinghuang@tcl.com
...
static ssize_t chg_ntc_debugfs_store(struct device *dev, struct device_attribute *attr,
					 const char *buf, size_t count)
{
	battery_temp = simple_strtol(buf, NULL, 10);
	printk("Usage: echo [temp] > sys/devices/platform/battery/ntc_debug: battery_temp = %d\n",battery_temp);

	return count;

}

static DEVICE_ATTR(ntc_debug, 0664, chg_ntc_debugfs_show, chg_ntc_debugfs_store);
#endif
//[BUGFIX]-Mod-END by SCDTABLET.jinghuang@tcl.com
...
static ssize_t store_chg_enable(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
...
static DEVICE_ATTR(chg_enable, 0644, show_chg_enable, store_chg_enable);
//[BUGFIX]-Mod-END by SCDTABLET.zhengding.chen@tcl.com
...
//[BUGFIX]-Mod-BEGIN by SCDTABLET.zhengding.chen@tcl.com,02/16/2016,PR-1590118
//modify NTC and bms 
//when soc = 100, ui_soc = 99, wait batfull,changge UI_SOC to 100
		if (!((BMT_status.SOC == 100)&&(BMT_status.UI_SOC == 99)))
			BMT_status.UI_SOC = BMT_status.SOC;
//[BUGFIX]-Mod-END by SCDTABLET.zhengding.chen@tcl.com,02/16/2016,PR-1590118
#else
...
PMU_STATUS do_batt_temp_state_machine(void)
{
	...
	// 41 < temperature < 53
	else if (BMT_status.temperature > MAX_CHARGE_TEMPERATURE_LOW_CURRENT) 
	{
		if (g_batt_temp_status == TEMP_POS_HIGH)
		{
			if(BMT_status.temperature > batt_cust_data.max_charge_temperature_minus_x_degree) {
				battery_log(BAT_LOG_CRTI,
				    "[BATTERY] Battery Temperature down from %d to %d(%d), not allow charging yet!!\n\r",
		...
		battery_log(BAT_LOG_CRTI,
			"[BATTERY] Battery Temperature down from %d to %d(%d), allow low cuurent charging !!\n\r",
			batt_cust_data.max_charge_temperature, BMT_status.temperature,
				    batt_cust_data.max_charge_temperature_minus_x_degree);
	...
	{
		battery_log(BAT_LOG_CRTI,"[BATTERY] Battery Temperature normal cuurent charging !!\n\r");
		g_batt_temp_status = TEMP_POS_NORMAL;
	}
	return PMU_STATUS_OK;
}
...
#endif
	//[BUGFIX]-Mod-END by SCDTABLET.jinghuang@tcl.com
//[BUGFIX]-Mod-BEGIN by SCDTABLET.jinghuang@tcl.com,01/06/2016,1312226,
//improve algorithm for low temperature charging
#ifdef SOC_SYNCHRO_UI_SOC_TIMER
...
//improve algorithm for low temperature charging
		...
		if (charge_status == KAL_TRUE) {
			...
			if((pre_sync_soc>=70)&&(pre_sync_soc<100)){
				
				if(step_flag)
				{
					printk("jinghuang:step_flag=%d,synchro_time=%d\n",step_flag,synchro_time);
					if(synchro_time>20)
						{
							time_count=(synchro_time/10-1)*10;
							
						}
					else if(synchro_time<10)
						synchro_time=synchro_time+10;
					if(synchro_time<=20)
						time_count=0;
					
			}
				printk("jinghuang:synchro_time=%d,time_count=%d,step_flag=%d\n",synchro_time,time_count,step_flag);
			//synchro_time=synchro_time-time_count;
			synchro_time = (cur_soc_synchro_time - pre_soc_synchro_time-time_count) % soc_synchro_timer;
				step_flag=0;
			
				}

			printk("jinghuang:<battery charge>  pre_sync_soc = %d, SOC = %d, cur_soc_synchro_time=%d,pre_soc_synchro_time=%d,soc_synchro_timer=%d,sync_time = %d, BMT_status.temperature = %d\n",
				pre_sync_soc, SOC, cur_soc_synchro_time,pre_soc_synchro_time,soc_synchro_timer,synchro_time, BMT_status.temperature);
...
/* [PLATFORM]-Add-BEGIN by TCTSZ.pingao, 09/09/2015,  PR-1207804, change charge current */
#ifdef CONFIG_TCT_LIMIT_USB_CURRENT
	if (BMT_status.charger_exist == KAL_TRUE) {
		if((BMT_status.charger_type == STANDARD_HOST)
			&& (BMT_status.ICharging >= batt_cust_data.usb_charger_current))
			BMT_status.ICharging = batt_cust_data.usb_charger_current - 15;
	}
#endif
...
static void mt_battery_notify_VBatTemp_check(void)
{
	...
	if (BMT_status.temperature >= batt_cust_data.max_raising_charge_temperature - 1) {
...
#else
#ifdef BAT_LOW_TEMP_PROTECT_ENABLE
	else if (BMT_status.temperature <= batt_cust_data.min_dropping_charge_temperature + 1) {
		g_BatteryNotifyCode |= (0x0020 | (!!BMT_status.charger_exist) << 8);
		battery_log(BAT_LOG_CRTI, "[BATTERY] bat_temp(%d) out of range(too low)\n",
				    BMT_status.temperature);	
	}
...
void do_chrdet_int_task(void)
{
	...
	/* [PLATFORM]-Add-BEGIN by TCTSZ.pingao, 09/09/2015,  PR-1207804, change charge current */
		...
		if (BMT_status.charger_exist == KAL_TRUE) {
			if (BMT_status.charger_type == STANDARD_CHARGER)
				BMT_status.ICharging = (batt_cust_data.ac_charger_current/100 - 15);
			else
				BMT_status.ICharging = (batt_cust_data.usb_charger_current/100 -15);
		}
	#endif
...
//add UISOC algorithm for low temperature battery charging curve.	
		...
		if ((upmu_is_chr_det() == KAL_TRUE) && (charge_status == KAL_FALSE)) {
			...
			printk("<battery>  pre_sync_soc = %d, charge_status = %d\n", pre_sync_soc, charge_status);
...
void check_battery_exist(void)
{
	...
	unsigned int i;
/*[FEATURE]-Add-BEGIN by huichen@tcl.com, 2015/11/30,mini sw pr 983534 */
