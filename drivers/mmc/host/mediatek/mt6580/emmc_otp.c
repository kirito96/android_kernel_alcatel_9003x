...
printk("***xbl***OTP:NON TD project ,call id_offset.\n");
    ...
    if(fd >= 0) {
        ...
        if (len < TRACE_REGION_SZ) {
            ...
            printk("[EMMC_OTP]read file failed\n");
            goto fail;
        } else {
            printk("[EMMC_OTP]read ok \n");
    ...
    if (bf == 0x0 || bf == 0x1) {
        printk("[EMMC_OTP]use id group %d\n", bf);
...
ready:
    ...
    printk(KERN_DEBUG "[EMMC_OTP]g_id_offset.offset:  0x%x\n", g_id_offset.offset); //zhi.li.sz
