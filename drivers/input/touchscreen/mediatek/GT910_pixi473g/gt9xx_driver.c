...
static unsigned int irq_work_counter = 0;
#define irq_work_counter_max 10000
...
static u8 tpd_proximity_flag_tmp = 0;
...
#ifdef TPD_PROXIMITY_CLOSETO_FACE
    return ret;
#endif
...
s32 gtp_i2c_read(struct i2c_client *client, u8 *buf, s32 len)
{
    ...
    {
#if GTP_GESTURE_WAKEUP
        if (gesture_wakeup_flag_tmp != 0 && DOZE_ENABLED == doze_status)
        {
            return ret;
        }
#endif
...
static irqreturn_t tpd_eint_interrupt_handler(void)
{
    TPD_DEBUG_PRINT_INT;
    irq_work_counter++;
    if(irq_work_counter > irq_work_counter_max) {
       irq_work_counter = 0;
    }
    tpd_flag = 1;
...
static void gtp_esd_check_func(struct work_struct *work)
{
    ...
    for (i = 0; i < 3; i++)
    {
        ...
        GTP_INFO("[Esd]0x8040 = 0x%02X, 0x8041 = 0x%02X ret = %d irq_work_counter = %d", esd_buf[0], esd_buf[1], ret, irq_work_counter);
        if (ret < 0)
        {
...
#ifndef TPD_PROXIMITY_CLOSETO_FACE
    u8 proximity_status;
#endif
...
#ifdef TPD_PROXIMITY_CLOSETO_FACE
           if (((finger & 0x40) == 0x40) || (finger & 0x0f) >= 2)
           {
               tpd_proximity_detect = 0;
               //GTP_INFO("Large touch was detected! tpd_proximity_detect\n");
           } else {
...
static s8 gtp_wakeup_sleep(struct i2c_client *client)
{
    ...
    if (CHIP_TYPE_GT9F == gtp_chip_type && !gesture_wakeup_flag_tmp)
    {
        u8 opr_buf[2] = {0};
...
static void tpd_suspend(struct device *h)
{
...
#ifdef TPD_PROXIMITY
    tpd_proximity_flag_tmp = tpd_proximity_flag;
    GTP_INFO("System suspend. tpd_proximity_flag = %d tpd_proximity_flag_tmp = %d tpd_halt = %d\n", tpd_proximity_flag, tpd_proximity_flag_tmp, tpd_halt);
    if (tpd_proximity_flag_tmp == 1)
    {
        return ;
    }
    ...
    gesture_wakeup_flag_tmp = gesture_wakeup_flag;
    GTP_INFO("System suspend. gesture_wakeup_flag_tmp = %d gesture_wakeup_flag = %d\n", gesture_wakeup_flag_tmp, gesture_wakeup_flag);
...
static void tpd_resume(struct device *h)
{
...
#ifdef TPD_PROXIMITY
    GTP_INFO("System resume. tpd_proximity_flag = %d tpd_proximity_flag_tmp = %d, tpd_halt = %d\n", tpd_proximity_flag, tpd_proximity_flag_tmp, tpd_halt);
    if (tpd_proximity_flag_tmp == 1)
    {
        return ;
    }
    ...
    GTP_INFO("System resume. gesture_wakeup_flag_tmp = %d gesture_wakeup_flag = %d\n", gesture_wakeup_flag_tmp, gesture_wakeup_flag);
    if(gesture_wakeup_flag_tmp != gesture_wakeup_flag) {
        GTP_ERROR("===== tpd_resume tp may not response ===\n");
    }
