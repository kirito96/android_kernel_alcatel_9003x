...
#define COMPATIABLE_NAME "mediatek,mxc4005"
...
static int MXC400X_CheckDeviceID(struct i2c_client *client) {
    ...
    GSE_LOG("check mxc400x device id addr = 0x%x\n", client->addr);
    memset(databuf, 0, sizeof(u8) * 10);
...
static int MXC400X_SetPowerMode(struct i2c_client *client, bool enable, bool isNeedDelay) {
    u8 databuf[2] = {0};
    int res = 0;
    //struct mxc400x_i2c_data *obj = i2c_get_clientdata(client);
...
static int mxc400x_init_client(struct i2c_client *client, int reset_cali) {
    ...
    res = MXC400X_SetDataFormat(client, enable_status ? MXC400X_RANGE_8G : MXC400X_RANGE_8G_SLEEP);
    if(res != MXC400X_SUCCESS) {
        return res;
    }

    res = MXC400X_SetPowerMode(client, enable_status, false);
    if(res != MXC400X_SUCCESS) {
        return res;
    }
...
//    }
    ...
    if(sensor_power == false) {
        res = MXC400X_SetPowerMode(client, true, true);
        if(res) {
            GSE_ERR("Power on MXC400X error %d!\n", res);
        }
        msleep(20);
    }
        ...
        if((buff_in == NULL) || (size_in < sizeof(int))) {
            ...
            if(((value == 0) && (sensor_power == false)) || ((value == 1) && (sensor_power == true))) {
                ...
                if (atomic_read(&priv->suspend) == 0) {
                    ...
                    err = MXC400X_SetPowerMode(priv->client, enable_status, true);
...
static long mxc400x_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg) {
    ...
    case GSENSOR_IOCTL_READ_SENSORDATA:
        ...
        MXC400X_SetPowerMode(client, true, true);
        MXC400X_ReadSensorData(client, strbuf, MXC400X_BUFSIZE);
        if(copy_to_user(data, strbuf, strlen(strbuf) + 1)) {
            err = -EFAULT;
            break;
...
static int mxc400x_suspend(struct i2c_client *client, pm_message_t msg) {
    ...
    if(msg.event == PM_EVENT_SUSPEND) {
        ...
        if((err = MXC400X_SetPowerMode(obj->client, false, false))) {
...
static int mxc400x_auto_init(void) {
    ...
    GSE_LOG("%s: gsensor_init_flag = %d \n", __func__, gsensor_init_flag);
